...
fn process(input: &str) -> usize {
    ...
    dijkstra(&grid, (0, 0), (grid.num_cols() - 1, grid.num_rows() - 1), 4, 10).unwrap()
}

fn dijkstra(grid: &Grid, start: Cordinate, dest: Cordinate, min_steps: usize, max_steps: usize) -> Option<usize> {
    ...
    distances.insert((start, Direction::S, 0), 0);
    ...
    }) = heap.pop()
    {
        ...
        for (neighbour_coordinate, neighbour_dir) in neighbours(coords, grid) {
            if (steps < min_steps && neighbour_dir != dir) || (dir.opposite() == neighbour_dir) {
                continue;
            }
            let next = State {
            ...
            if next.steps > max_steps {
                // Cant move more than max_steps times in the same direction
                continue;
            }
    ...
    println!("{iters} - Max X: {max_x}, Max Y: {max_y}");
    None
}
