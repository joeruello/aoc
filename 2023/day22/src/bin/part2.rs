...
fn process(input: &str) -> usize {
    ...
    bricks
        .iter()
        .map(|b| count_disintergrations(&b.i(), &supported_by, &supports))
        .sum()
}
...
fn has_space_below(
    ...
    for xy in brick.xy() {
        for potential_support in candiate_bricks.iter() {
            ...
            if potential_support.xy().contains(&xy)  {
                supported
                    .entry(idx)
                    .and_modify(|v| {
...
fn count_disintergrations(
    brick_idx: &usize,
    supported_by: &HashMap<usize, HashSet<usize>>,
    supports_map: &HashMap<usize, HashSet<usize>>,
) -> usize {
    let mut dislodged: HashSet<usize> = HashSet::from([*brick_idx]);
    let mut queue = VecDeque::from([brick_idx]);

    while let Some(idx) = queue.pop_front() {
        if let Some(supports) = supports_map.get(idx) {
            for c in supports {
                if let Some(supported_by_c) = supported_by.get(c) {
                    let filtered = supported_by_c
                        .iter()
                        .filter(|d|!dislodged.contains(d))
                        .collect_vec();
