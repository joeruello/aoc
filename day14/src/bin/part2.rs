...
fn process(input: &str) -> usize {
    ...
    let mut target = None;
    for i in (0..1000000000).progress() {
        if let Some((entry, idx)) = grid_cache.get(&grid) {
            println!("grid cache hit for {i} -> {idx}");
            let cycle_len = i - idx;
            let cycle_start = idx;
...
fn tilt_south(grid: &mut TooDee<char>) {
    for x in 0..grid.num_cols() {
        let mut col: Vec<char> = grid.col(x).cloned().collect();
        col.reverse();
        let mut new_col = collapse(&col);
        new_col.reverse();
        for (i, c) in grid.col_mut(x).enumerate() {
            *c = new_col[i];
        }
...
fn tilt_east(grid: &mut TooDee<char>) {
    let num_cols = grid.num_cols();
    for y in 0..grid.num_rows() {
        let mut col: Vec<char> = grid[y].to_vec();
        col.reverse();
